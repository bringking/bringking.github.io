...
task :generate => [:sass, :update_asset_versions, :jekyll]
...
task :gzip_css do
  ...
  system "gzip -9 #{styles_dir}/screen.css"
  #system "mv #{styles_dir}/screen.#{asset_version}.css{.gz,}"
end


desc "GZip All"
task :gzip => [:gzip_html, :gzip_css]
